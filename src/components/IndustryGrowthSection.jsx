...
const IndustryGrowthSection = () => {
      ...
      return (
        <div className="bg-white/95 p-2 border border-neutral-200 rounded-lg shadow-lg">
          ...
          <p className="text-xs font-medium" style={{ color: payload[0].payload.color }}>
            Value: <span className="text-neutral-900 font-black">${payload[0].value}B</span>
          </p>
        </div>
      );
    }
  ...
  return (
    ...
    <section className="relative py-12 px-4 font-sans bg-neutral-50 text-neutral-900 overflow-hidden border-t border-neutral-200">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        <motion.div
          ...
          className="mb-8"
        >
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-100 border border-emerald-200 text-emerald-700 text-[10px] font-black tracking-widest uppercase mb-4">
            <BarChart3 className="w-3 h-3" />
            <span>Market Intelligence</span>
          </div>
          
          <h2 className="text-3xl md:text-5xl font-black mb-3 leading-tight tracking-tight text-neutral-900">
            Switch to the <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-teal-500 to-cyan-600">$133 Billion</span> Data Industry
          </h2>
        </motion.div>

        {/* Chart Container: Reduced padding from p-12 to p-6 */}
        <motion.div
          initial={{ opacity: 0, scale: 0.98 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          className="bg-white border border-neutral-200 rounded-[1.5rem] p-5 md:p-8 shadow-xl mb-10 max-w-3xl mx-auto"
        >
          <div className="flex items-center justify-between mb-4 border-b border-neutral-100 pb-2">
            ...
            <span className="text-[10px] font-black text-neutral-400 uppercase tracking-tighter">CAGR: ~25%</span>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={60}>
                  ...
                  <LabelList dataKey="label" position="top" fill="#404040" fontWeight="900" fontSize={14} offset={8} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* BOTTOM CTA: Scaled down internal components */}
        <motion.div
        ...
        >
            <a href={whatsappLink} target="_blank" rel="noreferrer" className="w-full max-w-md relative z-10 group">
                <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="relative w-full bg-gradient-to-b from-[#00d647] to-[#009933] text-white text-lg md:text-xl font-black py-4 px-6 rounded-2xl shadow-lg flex flex-col items-center justify-center gap-1 overflow-hidden border-t border-green-300/30"
                >
                    <div className="flex items-center gap-2">
                        <span>Register Now for £99</span>
                        <span className="text-green-900 line-through decoration-red-600 opacity-60 text-sm">£299</span>
                    </div>
